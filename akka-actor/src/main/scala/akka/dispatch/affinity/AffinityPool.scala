...
private[akka] final class AffinityPoolConfigurator(config: Config, prerequisites: DispatcherPrerequisites)
  ...
  private val queueSelectorFactory: QueueSelectorFactory =
    prerequisites.dynamicAccess.createInstanceFor[QueueSelectorFactory](queueSelectorFactoryFQCN, immutable.Seq(classOf[Config] → config.getConfig("fair-work-distribution")))
      .recover({
...
trait QueueSelector {
  /**
   * @eturns given a `Runnable` a number between 0 .. `queues` (exclusive)
...
private[akka] final class FairDistributionHashCache( final val config: Config) extends QueueSelectorFactory {
  ...
  private[this] val fairDistributionThreshold = config.getInt("threshold").requiring(thr ⇒
